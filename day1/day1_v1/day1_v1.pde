...
void secondPuzzle(){
  ...
  list = split(input, " ");
  pastFrequencies = new IntList();
  
  while(!pastFrequencies.hasValue(frequency)){
    pastFrequencies.append(frequency);
    
    //add an input at the current index
    frequency += int(list[index]);
    
    //increase index if not over max
  ...
  println(frequency + " completed in " + completionTime + " milliseconds");
}
